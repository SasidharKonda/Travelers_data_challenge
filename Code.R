...
# zip_freq_table = as.data.frame(table(Full_Dataset$zip.code))
# Full_Dataset$zip.code[is.na(Full_Dataset$zip.code)] = zip_freq_table$Var1[which.max(zip_freq_table$Freq)]
# x = zip_freq_table$Var1[which.max(zip_freq_table$Freq)]
# #Function to append Z-scores for premium, len.at.res, tenure to the dataset
# Full_Dataset[is.na(Full_Dataset$zip.code),]
...
#View(Full_Dataset[is.na(Full_Dataset$state),])
...
unique(Full_Dataset$zip.code)[unique(Full_Dataset$zip.code)%in%unique(zipcode_req$zipcode.zip)==FALSE]

colnames(zipcode_req) = c('zip.code','city','state')
Full_Dataset = merge(Full_Dataset,zipcode_req,all.x = TRUE)
...
write.csv(validation_set_dummies,"E:/Travellers Case Competetion/Training_set.csv")
